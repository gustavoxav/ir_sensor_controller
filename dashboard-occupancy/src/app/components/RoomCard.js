    ...
    <div className="glass-card rounded-2xl p-6 animate-fade-in hover:shadow-xl transition-all duration-300 group">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          >
            <svg
              className={`w-6 h-6 ${isOccupied ? "text-white" : "text-muted-foreground"
                }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-muted/30">
          <div className="flex items-center gap-2">
            <svg
              className={`w-4 h-4 ${isLightOn ? "text-warning" : "text-muted-foreground"
                }`}
          ...
          <div className="flex items-center gap-2">
            <span
              className={`status-indicator ${isLightOn ? "status-on" : "status-off"
                }`}
            />
            <span
              className={`text-sm font-bold ${isLightOn ? "text-success" : "text-danger"
                }`}
            >
              {isLightOn ? "ON" : "OFF"}
            </span>
        ...
        <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-muted/30">
          ...
          <span
            className={`text-sm font-bold ${last_event === "entrada" ? "text-success" : "text-danger"
              }`}
          >
